const CURRENCIES = {
    fish: {
        ...
        get gain() {
            ...
            x = x.pow(sharkUpgEffect('s4')).pow(getPAEffect(0))
    ...
    prestige: {
        ...
        get gain() {
            ...
            var exp = 0.05
            exp += getPAEffect(7).toNumber()
            if (hasResearch('p4')) exp += 0.05
            if (hasEvolutionGoal(5)) exp += 0.0125

            x = expPow(x,exp).pow(coreReactorEffect(1)).mul(getSharkBonus("prestige")).mul(getPAEffect(2))
